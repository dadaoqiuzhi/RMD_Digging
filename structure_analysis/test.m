...
for ijk = 1:size(bondoutdata,1)
    ...
    if strcmpi(first_line_OK,'y') %开始搜索建立等深度拓扑结构
        ...
        [table_TopoBond,topo_deep_OK] = TopoBond(bondoutdata,ijk,topo_deep,group_target_copy,table_line_mark,n_bond); %根据group文件深度从bonds.*文件考察每行开始建立等深的拓扑文件，深度不够则不满足要求
